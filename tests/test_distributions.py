...
def test_logistic_bernoulli(device):
    ...
    z = dist.rsample([N])
    assert torch.allclose(z.mean(0), dist.mean, atol=1e-3)
    assert torch.allclose(z.std(0), dist.stddev, atol=1e-3)
    b = dist.threshold(z)
    assert torch.allclose(b.mean(0), probs, atol=1e-3)
    zz = dist.csample(b)
    assert torch.allclose(dist.threshold(zz), b)
    # E_b[E_{z|b}[z]] = E_z[z]
    assert torch.allclose(zz.mean(0), dist.mean, atol=1e-3)
    assert torch.allclose(zz.std(0), dist.stddev, atol=1e-3)
